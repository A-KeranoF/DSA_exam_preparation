...
{
    // TODO ADD MORE BULLSHIT
    for (size_t i = headIndex; i != tailIndex; moveIndexForwards(i, capacity))
...
{
    // TODO check size
    T* temp = allocator.allocate(newCapacity);

    size_t oldSize = size();

    for (size_t i = 0; i < capacity; ++i) {
        temp[i] = allocator.construct(&front(), front());
        popFront();
    }
    ...
    free(); // because i do not want to copy-paste the same code
    data = temp;
}
...
void ArrayDeque<T>::popFront()
{
    ...
    allocator.destroy(data + headIndex);
...
void ArrayDeque<T>::popBack()
{
    ...
    allocator.destroy(data + tailIndex);
    --sz;
...
void ArrayDeque<T>::pushFront(const T& element)
{
    ...
    allocator.construct(&data[headIndex], element);
    ++sz;
}
...
void ArrayDeque<T>::pushBack(const T& element)
{
    ...
    allocator.construct(&data[tailIndex], element);
